  ...
  <div style={{ ...card, marginBottom: '0.5rem', padding: '0.875rem 1.25rem' }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
      ...
      <span style={{ fontSize: 18, fontWeight: 700, color:
        sig.signalType === 'BUY' ? '#22c55e' :
        sig.signalType === 'SELL' ? '#ef4444' :
        sig.signalType === 'WATCH' ? '#eab308' : '#444'
      }}>{sig.score}/100</span>
    </div>
            ...
            {filtered.map((d, i) => (
              <tr key={i} style={{ borderBottom: '0.5px solid rgba(255,255,255,0.04)' }}
                ...
                <td style={{ padding: '7px 10px' }}>
                  <span style={{ color: d.buySell === 'BUY' ? '#22c55e' : '#ef4444', fontWeight: 600 }}>
                    {d.buySell}
                  </span>
                </td>
    ...
    <div>
      ...
      {data && (
        <>
          ...
          {view === 'summaries' && (
            <div>
              {(data.allSummaries || []).map((s, i) => <SummaryCard key={i} s={s} />)}
            </div>
          )}
